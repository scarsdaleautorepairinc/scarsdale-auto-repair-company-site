...
def income_report(conn, period, selected):
    ...
    payments = [dict(row) for row in conn.execute('''
        ...
        FROM repair_orders ro JOIN customers c ON c.id=ro.customer_id
        JOIN vehicles v ON v.id=ro.vehicle_id
        WHERE ro.paid_at >= ? AND ro.paid_at < ? ORDER BY ro.paid_at, ro.id
    ''', boundaries)]
    ...
    return {
        ...
        'totals': {key: sum(bucket[key] for bucket in buckets.values()) for key in ('visits', 'paid_tickets', 'income_cents', 'missing_amounts')},
